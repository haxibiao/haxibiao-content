...
class CacheResponse
{
	...
	{
		return $request->isMethod('GET') && $response->getStatusCode() == 200 && $this->isCrawler();
	}
	...
	protected function isCrawler(){
		...
		foreach ([
			...
			if (isset($httpHeadersArr[$altHeader])) {
				$userAgent .= $httpHeadersArr[$altHeader].' ';
			}
		}
	...
	private function compiledExclusions(){
		$data = array(
			'Safari.[\d\.]*',
			'Firefox.[\d\.]*',
			' Chrome.[\d\.]*',
		...
		return $this->compileRegex($data);
	}

	private function compiledRegex(){
		$data = array(
			' YLT',
			'^b0t$',
			'^bluefish ',
	...
	private function compileRegex($patterns)
	{
