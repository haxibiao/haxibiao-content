<div class="note-info info-lg">
    <a class="avatar-category" href="{{ $category->checkAdmin() ? "/category/$category->id/edit" : 'javascript:;' }}"
        ...
        <img src="{{ $category->logoUrl }}" alt="{{ $category->id }}:{{ $category->name }}">
    </a>
    <div class="btn-wrap">
        ...
        @if (Auth::check())
            <a class="btn-base btn-hollow" data-target=".modal-contribute" data-toggle="modal"
                href="javascript:;">{{ $category->isAdmin(Auth::user()) ? '收录' : '投稿' }}</a>
        @endif
    </div>
    <div class="title">
    ...
    <div class="info">
        收录了{{ $category->count_articles }}篇作品 · {{ $category->count_follows }}人关注
    </div>
